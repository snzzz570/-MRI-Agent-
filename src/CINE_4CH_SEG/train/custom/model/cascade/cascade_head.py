...
@HEADS.register_module()
class DYSegPyramid_Head_R(nn.Module):
    def __init__(
        self,
        in_channels: int,
    ):
        super(DYSegPyramid_Head_R, self).__init__()
        self.conv_bin1_1 = nn.Conv3d(in_channels, 1, 1) # 第一个二值图 第一种分辨率下
        self.conv_bin2_1 = nn.Conv3d(in_channels, 1, 1) # 第一个二值图 第二种分辨率下

        self.conv_bin1_2 = nn.Conv3d(in_channels, 1, 1)
        self.conv_bin2_2 = nn.Conv3d(in_channels, 1, 1)

        self.conv_bin1_3 = nn.Conv3d(in_channels, 1, 1)
        self.conv_bin2_3 = nn.Conv3d(in_channels, 1, 1)
    ...
    def forward(self, input1, input2):
        ...
        pred_bin1_3 = self.conv_bin1_3(inputs)


        inputs = F.interpolate(input2, scale_factor=(1.0,) * 3, mode="trilinear", align_corners=False)
        pred_bin2_1 = self.conv_bin2_1(inputs)
        pred_bin2_2 = self.conv_bin2_2(inputs)
        pred_bin2_3 = self.conv_bin2_3(inputs)


        return pred_bin1_1, pred_bin2_1, pred_bin1_2, pred_bin2_2, pred_bin1_3, pred_bin2_3

    def loss(self, inputs, target1, target2, target3):

        loss = []
        weight_ratios = [1, 10, 1, 10, 1, 10]
        ...
        for idx, pred in enumerate(inputs[:2]):
            ...
            loss.append(
                self._loss_unit(
                    pred, tgt, "loss1_bin_" + str(idx), restore_thresh[idx], 4
                )
            )
        
        for idx, pred in enumerate(inputs[2:4]):
            ...
            loss.append(
                self._loss_unit(
                    pred, tgt, "loss2_bin_" + str(idx), restore_thresh[idx], 5
                )
            )

        for idx, pred in enumerate(inputs[4:]):
            tgt = target3[:, idx : (idx + 1)]
            loss.append(
                self._loss_unit(
                    pred, tgt, "loss3_bin_" + str(idx), restore_thresh[idx], 3
                )
            )
    ...
    def _loss_unit(self, inputs, targets, idx, restore_thresh, alpha):
        with torch.no_grad():
            targets = (targets > restore_thresh) * 1.0
            weight = torch.ones_like(targets)
            weight = weight + (((targets > 0).float()) * alpha)
...
@HEADS.register_module()
class DYSegPyramid_Head_L(nn.Module):
    def __init__(
        self,
        in_channels: int,
    ):
        super(DYSegPyramid_Head_L, self).__init__()
        self.conv_bin1_1 = nn.Conv3d(in_channels, 1, 1) # 第一个二值图 第一种分辨率下
        self.conv_bin2_1 = nn.Conv3d(in_channels, 1, 1) # 第一个二值图 第二种分辨率下
    ...
    def loss(self, inputs, target1, target2, target3):
        ...
        for idx, pred in enumerate(inputs[:2]):
            ...
            loss.append(
                self._loss_unit(
                    pred, tgt, "loss1_bin_" + str(idx), restore_thresh[idx], 2
                )
            )
        ...
        for idx, pred in enumerate(inputs[2:4]):
            ...
            loss.append(
                self._loss_unit(
                    pred, tgt, "loss2_bin_" + str(idx), restore_thresh[idx], 3
                )
            )
