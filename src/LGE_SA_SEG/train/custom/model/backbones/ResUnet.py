...
class SimpleCoxHead(nn.Module):
    ...
    def forward(self, x):
        # x: (batch_size, in_features) → 风险分数: (batch_size,)
        return self.fc(x).squeeze(1)  # 移除最后一个维度，便于计算损失
...
class CNNTrans_multi(nn.Module):
    def __init__(self, in_ch=1, channels=32, blocks=3):
        ...
        self.deep_sup_cls = nn.Linear(128, 5)
        # self.deep_sup_cls_flow = nn.Linear(128, 1)
        # self.deep_sup_reg = nn.Linear(128, 1)
...
@BACKBONES.register_module()
class DenseTrans_multi2d(nn.Module):
    def __init__(self, in_ch=1, channels=32, blocks=3):
        super(DenseTrans_multi2d, self).__init__()
        self.CNN = models.densenet121(pretrained=False)
        self.CNN.fc = nn.Sequential(nn.Linear(1000, 128))
        self.transformer = Transformer(dim=128, depth=3, heads=4, dim_head=32, mlp_dim=256, dropout=0.1)
        self.mlp_head_cls = nn.Sequential(
            nn.LayerNorm(128),
            nn.Linear(128, 4)
...
class CNNTrans_cox(nn.Module):
    def __init__(self, in_ch=1, channels=32, blocks=3):
        ...
        self.transformer = Transformer(dim=128, depth=3, heads=4, dim_head=32, mlp_dim=256, dropout=0.2)
...
@BACKBONES.register_module()
class CNNTrans_cox_cli(nn.Module):
    def __init__(self, in_ch=1, channels=32, blocks=3):
        super(CNNTrans_cox_cli, self).__init__()
        self.CNN = ResUnet_enc(in_ch=in_ch, channels=channels, blocks=blocks)
        self.CNN.fc = nn.Sequential(nn.Linear(channels * 8, 128),
                nn.ReLU(),
                nn.BatchNorm1d(128))
        ...
        self.cli_encoder = nn.Sequential(
            nn.Linear(21, 64),
            nn.ReLU(),
            nn.Dropout(0.2),
            nn.Linear(64, 128),
        ...
        self.cli_cls = nn.Linear(128, 1)
        self.deep_sup_cls = nn.Linear(128, 1)
        self.cox_regression = SimpleCoxHead(128)
        # self.deep_sup_cls_flow = nn.Linear(128, 1)
        # self.deep_sup_reg = nn.Linear(128, 1)


    def forward(self, img, cli_text):
        # import time
        # import SimpleITK as sitk
        # s = str(time.time())
        # print(img.shape)
        ...
        x = self.CNN.fc(x)
        deep_sup_cls = self.deep_sup_cls(x)
        x_cli = self.cli_encoder(cli_text)
        ...
        # print(F_CNN.shape)
        # x = self.dropout(F_CNN.unsqueeze(1))
        x = self.transformer(x.unsqueeze(1) + x_cli.unsqueeze(1))
        x_cox = self.cox_regression(x)

        x_cls = self.mlp_head_cls(x)
        x_cli_cls = self.cli_cls(x_cli)
        x_cli_cox = self.cox_regression(x_cli)
        # x_reg = self.mlp_head_reg(x)
        # print(x_cls.shape, x_reg.shape, deep_sup_cls.shape, deep_sup_reg.shape)
        return x_cls, x_cox, deep_sup_cls, x_cli_cls, x_cli_cox
