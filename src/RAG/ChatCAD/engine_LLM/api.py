...
class Chat_api:
    def __init__(self, api_key: str, proxy: Optional[str] = None, verbose: bool = False, 
        ...
        :param proxy: Optional proxy URL (deprecated in openai>=1.0.0).
        :param verbose: If True, print prompts and responses.
        :param model: The OpenAI model to use (e.g., "gpt-4o", "deepseek-chat", "qwen-plus").
        :param base_url: API base URL (optional, for DeepSeek/Qwen etc.)
        """
        ...
        self.proxy = proxy  # Note: proxy handling may need httpx configuration
...
def answer_quest(quest: str, api_key: str, topic_base_dict: list, 
                 model: str = "gpt-4o", base_url: str = None):#,topic):
    """
    使用 LLM 从知识库检索答案
    ...
    # 初始化 Chat_api (兼容 openai>=1.0.0)
    chatapi=Chat_api(api_key=api_key, verbose=False, proxy=None, model=model, base_url=base_url)
